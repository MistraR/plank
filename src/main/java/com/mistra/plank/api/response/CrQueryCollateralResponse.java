package com.mistra.plank.api.response;

public class CrQueryCollateralResponse extends BaseTradeResponse {

    /**
     * 成本价格
     */
    private String Cbjg;
    /**
     * 仓位比例
     */
    private String Cwbl;
    private String Dwc;
    /**
     * 浮动盈亏
     */
    private String Fdyk;
    /**
     * 股东代码
     */
    private String Gddm;
    /**
     * 可用数量
     */
    private String Gfky;
    private String Gfye;
    private String Khyq;
    /**
     * 市场
     */
    private String Market;
    /**
     * 融资买入可用数量
     */
    private String Rzmrgfky;
    private String Rzmrgfye;
    /**
     * 是否为担保品
     */
    private String Sfwdbp;
    /**
     * 市值
     */
    private String Sz;
    /**
     * 盈亏
     */
    private String Yk;
    /**
     * 盈亏比例
     */
    private String Ykbl;
    /**
     * 资金帐号
     */
    private String Zjzh;
    /**
     * 证券代码
     */
    private String Zqdm;
    /**
     * 证券名称
     */
    private String Zqmc;
    /**
     * 证券数量
     */
    private String Zqsl;
    /**
     * 最新价格
     */
    private String Zxjg;
    private String stktype;

    public String getCbjg() {
        return Cbjg;
    }

    public void setCbjg(String cbjg) {
        Cbjg = cbjg;
    }

    public String getCwbl() {
        return Cwbl;
    }

    public void setCwbl(String cwbl) {
        Cwbl = cwbl;
    }

    public String getDwc() {
        return Dwc;
    }

    public void setDwc(String dwc) {
        Dwc = dwc;
    }

    public String getFdyk() {
        return Fdyk;
    }

    public void setFdyk(String fdyk) {
        Fdyk = fdyk;
    }

    public String getGddm() {
        return Gddm;
    }

    public void setGddm(String gddm) {
        Gddm = gddm;
    }

    public String getGfky() {
        return Gfky;
    }

    public void setGfky(String gfky) {
        Gfky = gfky;
    }

    public String getGfye() {
        return Gfye;
    }

    public void setGfye(String gfye) {
        Gfye = gfye;
    }

    public String getKhyq() {
        return Khyq;
    }

    public void setKhyq(String khyq) {
        Khyq = khyq;
    }

    public String getMarket() {
        return Market;
    }

    public void setMarket(String market) {
        Market = market;
    }

    public String getRzmrgfky() {
        return Rzmrgfky;
    }

    public void setRzmrgfky(String rzmrgfky) {
        Rzmrgfky = rzmrgfky;
    }

    public String getRzmrgfye() {
        return Rzmrgfye;
    }

    public void setRzmrgfye(String rzmrgfye) {
        Rzmrgfye = rzmrgfye;
    }

    public String getSfwdbp() {
        return Sfwdbp;
    }

    public void setSfwdbp(String sfwdbp) {
        Sfwdbp = sfwdbp;
    }

    public String getSz() {
        return Sz;
    }

    public void setSz(String sz) {
        Sz = sz;
    }

    public String getYk() {
        return Yk;
    }

    public void setYk(String yk) {
        Yk = yk;
    }

    public String getYkbl() {
        return Ykbl;
    }

    public void setYkbl(String ykbl) {
        Ykbl = ykbl;
    }

    public String getZjzh() {
        return Zjzh;
    }

    public void setZjzh(String zjzh) {
        Zjzh = zjzh;
    }

    public String getZqdm() {
        return Zqdm;
    }

    public void setZqdm(String zqdm) {
        Zqdm = zqdm;
    }

    public String getZqmc() {
        return Zqmc;
    }

    public void setZqmc(String zqmc) {
        Zqmc = zqmc;
    }

    public String getZqsl() {
        return Zqsl;
    }

    public void setZqsl(String zqsl) {
        Zqsl = zqsl;
    }

    public String getZxjg() {
        return Zxjg;
    }

    public void setZxjg(String zxjg) {
        Zxjg = zxjg;
    }

    public String getStktype() {
        return stktype;
    }

    public void setStktype(String stktype) {
        this.stktype = stktype;
    }

}
