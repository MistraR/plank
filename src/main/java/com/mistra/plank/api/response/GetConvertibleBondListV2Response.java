package com.mistra.plank.api.response;

public class GetConvertibleBondListV2Response extends BaseTradeResponse {

    private int ExStatus;
    private boolean ExIsToday;
    private String BONDCODE;
    private String BONDNAME;
    private String BUYREDEMCODE;
    private String BUYREDEMNAME;
    private String BUYREDEMPRIC;
    private String CREDITRATING;
    private String Cybbz;
    private String FLOORBUYVOL;
    private String ISSUESDATE;
    private String ISSUEVOL;
    private String LIMITBUYVOL;
    private String LWRANDATE;
    private String Market;
    private String PARVALUE;
    private String PLACINGCODE;
    private String PLACINGNAME;
    private String PLACINGRIGHT;
    private String PURCHASEDATE;
    private String RATING;
    private String SFZCZ;
    private String SUBCODE;
    private String SUBNAME;
    private String SWAPPRICE;
    private String SWAPSCODE;
    private String SWAPSNAME;
    private String SWAPVALUE;
    private String BONDTYPECODE;

    public int getExStatus() {
        return ExStatus;
    }

    public void setExStatus(int exStatus) {
        ExStatus = exStatus;
    }

    public boolean isExIsToday() {
        return ExIsToday;
    }

    public void setExIsToday(boolean exIsToday) {
        ExIsToday = exIsToday;
    }

    public String getBONDCODE() {
        return BONDCODE;
    }

    public void setBONDCODE(String bONDCODE) {
        BONDCODE = bONDCODE;
    }

    public String getBONDNAME() {
        return BONDNAME;
    }

    public void setBONDNAME(String bONDNAME) {
        BONDNAME = bONDNAME;
    }

    public String getBUYREDEMCODE() {
        return BUYREDEMCODE;
    }

    public void setBUYREDEMCODE(String bUYREDEMCODE) {
        BUYREDEMCODE = bUYREDEMCODE;
    }

    public String getBUYREDEMNAME() {
        return BUYREDEMNAME;
    }

    public void setBUYREDEMNAME(String bUYREDEMNAME) {
        BUYREDEMNAME = bUYREDEMNAME;
    }

    public String getBUYREDEMPRIC() {
        return BUYREDEMPRIC;
    }

    public void setBUYREDEMPRIC(String bUYREDEMPRIC) {
        BUYREDEMPRIC = bUYREDEMPRIC;
    }

    public String getCREDITRATING() {
        return CREDITRATING;
    }

    public void setCREDITRATING(String cREDITRATING) {
        CREDITRATING = cREDITRATING;
    }

    public String getCybbz() {
        return Cybbz;
    }

    public void setCybbz(String cybbz) {
        Cybbz = cybbz;
    }

    public String getFLOORBUYVOL() {
        return FLOORBUYVOL;
    }

    public void setFLOORBUYVOL(String fLOORBUYVOL) {
        FLOORBUYVOL = fLOORBUYVOL;
    }

    public String getISSUESDATE() {
        return ISSUESDATE;
    }

    public void setISSUESDATE(String iSSUESDATE) {
        ISSUESDATE = iSSUESDATE;
    }

    public String getISSUEVOL() {
        return ISSUEVOL;
    }

    public void setISSUEVOL(String iSSUEVOL) {
        ISSUEVOL = iSSUEVOL;
    }

    public String getLIMITBUYVOL() {
        return LIMITBUYVOL;
    }

    public void setLIMITBUYVOL(String lIMITBUYVOL) {
        LIMITBUYVOL = lIMITBUYVOL;
    }

    public String getLWRANDATE() {
        return LWRANDATE;
    }

    public void setLWRANDATE(String lWRANDATE) {
        LWRANDATE = lWRANDATE;
    }

    public String getMarket() {
        return Market;
    }

    public void setMarket(String market) {
        Market = market;
    }

    public String getPARVALUE() {
        return PARVALUE;
    }

    public void setPARVALUE(String pARVALUE) {
        PARVALUE = pARVALUE;
    }

    public String getPLACINGCODE() {
        return PLACINGCODE;
    }

    public void setPLACINGCODE(String pLACINGCODE) {
        PLACINGCODE = pLACINGCODE;
    }

    public String getPLACINGNAME() {
        return PLACINGNAME;
    }

    public void setPLACINGNAME(String pLACINGNAME) {
        PLACINGNAME = pLACINGNAME;
    }

    public String getPLACINGRIGHT() {
        return PLACINGRIGHT;
    }

    public void setPLACINGRIGHT(String pLACINGRIGHT) {
        PLACINGRIGHT = pLACINGRIGHT;
    }

    public String getPURCHASEDATE() {
        return PURCHASEDATE;
    }

    public void setPURCHASEDATE(String pURCHASEDATE) {
        PURCHASEDATE = pURCHASEDATE;
    }

    public String getRATING() {
        return RATING;
    }

    public void setRATING(String rATING) {
        RATING = rATING;
    }

    public String getSFZCZ() {
        return SFZCZ;
    }

    public void setSFZCZ(String sFZCZ) {
        SFZCZ = sFZCZ;
    }

    public String getSUBCODE() {
        return SUBCODE;
    }

    public void setSUBCODE(String sUBCODE) {
        SUBCODE = sUBCODE;
    }

    public String getSUBNAME() {
        return SUBNAME;
    }

    public void setSUBNAME(String sUBNAME) {
        SUBNAME = sUBNAME;
    }

    public String getSWAPPRICE() {
        return SWAPPRICE;
    }

    public void setSWAPPRICE(String sWAPPRICE) {
        SWAPPRICE = sWAPPRICE;
    }

    public String getSWAPSCODE() {
        return SWAPSCODE;
    }

    public void setSWAPSCODE(String sWAPSCODE) {
        SWAPSCODE = sWAPSCODE;
    }

    public String getSWAPSNAME() {
        return SWAPSNAME;
    }

    public void setSWAPSNAME(String sWAPSNAME) {
        SWAPSNAME = sWAPSNAME;
    }

    public String getSWAPVALUE() {
        return SWAPVALUE;
    }

    public void setSWAPVALUE(String sWAPVALUE) {
        SWAPVALUE = sWAPVALUE;
    }

    public String getBONDTYPECODE() {
        return BONDTYPECODE;
    }

    public void setBONDTYPECODE(String bONDTYPECODE) {
        BONDTYPECODE = bONDTYPECODE;
    }

}
